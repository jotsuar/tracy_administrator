...
class Cliente_controller extends CI_Controller 
{
	...
	{
		$data['ok'] 		= 	$ok;
		$data['title'] 		= 	"Consulta Cliente";
		$data['action']		= 	"usuario/cliente/consultar";
		$data['fecha']		=	TRUE;
	    $data['action2']	=	"usuario/cliente/consultar";
		
		$this->template->write_view('content','usuario_consulta_view', $data, TRUE);
	...
	public function registrar()
	{
		...
		if (!$this->form_validation->run()) {
			...
			$respuesta = $this->usuario_model->insertar_cliente(
				$tipo, $identificacion, $nombre, $apellidos, $telefono, $celular, $fecha, $correo, $rol
			);
	...
	public function consultar()
	{
		...
		if(!$this->form_validation->run()) {
			...
			$resultado['action'] 	= 	"usuario/cliente/consultar";
	...
	public function buscar($id, $ok = '')
	{
		...
		if(!$resultado['datos']) {
			...
			$resultado['action'] 	= 	"usuario/cliente/actualizar";
			$resultado['fecha'] 	= 	TRUE;
	...
	public function actualizar()
	{
		...
		if (!$this->form_validation->run()) {
			...
			$respuesta = $this->usuario_model->actualizar_cliente(
				$id, $tipo, $identificacion, $nombre, $apellidos, $telefono, $celular, $fecha, $correo
			);
