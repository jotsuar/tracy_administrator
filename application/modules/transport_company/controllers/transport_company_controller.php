...
class Transport_company_controller extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->template->write_view('content_header', 'transport_company/transporte_index_view');
		$this->load->model('transport_company_model');
	...
	{
		$data['ok'] = $ok;
		$this->template->write_view('content', 'transport_company/transporte_register_view', $data);
	...
	public function registrar()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$ok = $this->transport_company_model->insertar(
	...
	public function modificar()
	{
		...
		if (!$this->form_validation->run()) {
			...
			$datos['ok'] = $this->transport_company_model->modificar(
	...
	public function consultar()
	{
		...
		if (!$this->form_validation->run()) {
			$datos['transportes'] = $this->transport_company_model->see_all();
		} else {
			$datos['transportes'] = $this->transport_company_model->search(
	...
	public function buscar($id, $datos = NULL)
	{
		...
		$datos['transporte'] = $this->transport_company_model->buscar($id);
		if(!isset($datos['transporte']) || empty($datos['transporte'])) {
			redirect("/transporte/consultar");
		}
		$this->template->write_view('content', 'transport_company/transporte_actualizar_view', $datos);
		$this->template->render();		
	}

	public function validate($data, $field)
	{
		return $this->transport_company_model->validate(
			$data, 
			$field, 
			$this->input->post('id', TRUE)
		);
	...
	{
		return $this->transport_company_model->validate_vehicle(
			$data, 
			$field, 
			$this->input->post('id', TRUE)
		);
	...
	{
		$datos['empresas'] 				= 	$this->transport_company_model->see_all();
		$datos['tipo_vehiculos'] 	= 	$this->transport_company_model->see_all_tipo_vehiculo();
		...
		if (!$this->form_validation->run()) {
			...
			$datos['ok'] = $this->transport_company_model->insertar_vehiculo(
		...
		$this->template->write_view('content', 'transport_company/vehiculo/vehiculo_register_view', $datos);
		$this->template->render();
	}
	...
	{			
		...
		if (!$this->form_validation->run()) {
			...
			$datos['ok'] = $this->transport_company_model->modificar_vehiculo(
				strtoupper($this->input->post('placa', TRUE)),
				(int) $this->input->post('tipo', TRUE),
				$this->input->post('descripcion', TRUE),
				(int) $cupos = $this->input->post('cupos', TRUE),
	...
	public function consultar_vehiculo()
	{
		...
		if(!$this->form_validation->run()) {
			$datos['vehiculos'] = $this->transport_company_model->see_all_vehicles();
		} else {
			$datos['vehiculos'] = $this->transport_company_model->consultar_vehiculo(
				strtoupper($this->input->post('placa', TRUE))
			);
		}
		$this->template->write_view('content','transport_company/vehiculo/vehiculo_consultar_view', $datos);
		$this->template->render();
	}

	public function buscar_vehiculo($id, $datos = NULL)
	{
		$datos['vehiculo'] = $this->transport_company_model->buscar_vehiculo($id);
		...
		if (!isset($datos['vehiculo']) || empty($datos['vehiculo'])) {
			...
			$datos['empresas'] 				= 	$this->transport_company_model->see_all();
			$datos['tipo_vehiculos'] 	= 	$this->transport_company_model->see_all_tipo_vehiculo();
			$this->template->write_view('content','transport_company/vehiculo/vehiculo_actualizar_view', $datos);
		}
	...
	public function registrar_tipo_vehiculo()
	{
		...
		if (!$this->form_validation->run()) {
			...
			$respuesta = $this->transport_company_model->insertar_tipo_vehiculo(
		...
		$this->template
		->write_view('content','transport_company/vehiculo/tipo_vehiculo/tipo_vehiculo_register_view', $data);
	...
	public function modificar_tipo_vehiculo()
	{
		...
		if (!$this->form_validation->run()) {
			...
			$datos['ok'] = $this->transport_company_model->modificar_tipo_vehiculo(
				$this->input->post('id', TRUE),
	...
	{
		$datos['tipos'] = $this->transport_company_model->see_all_tipo_vehiculo();
		$this->template->write_view(
			'content', 
			'transport_company/vehiculo/tipo_vehiculo/tipo_vehiculo_consultar_view', 
			$datos
		);
	...
	{
		$datos['tipo'] = $this->transport_company_model->see_tipo_vehiculo($id);
		if (!$datos['tipo']) {
			redirect('/transporte/consultar_tipo_vehiculo');
		}
		$this->template->write_view(
			'content', 
			'transport_company/vehiculo/tipo_vehiculo/tipo_vehiculo_actualizar',
			$datos
		);
	...
	{
		return $this->transport_company_model->validate_tipo_vehiculo(
			$this->input->post('id', TRUE),
			$field,
			strtoupper($data)
		);
